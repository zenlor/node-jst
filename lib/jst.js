...
jst = module.exports = {
  compile: function(dir, output, callback) {
    ...
    walk(resolved_path, function(err, templates) {
      ...
      compiled.push(jst.compilers[jst.compiler].head.replace(/\{_ns\}/g, jst.namespace));
      templates.forEach(function (t) {
        ...
        if (/^\_/.test(name)) {
          compiled.push(jst.compilers[jst.compiler].partial({
            ns: jst.namespace, name: name.substring(1), tpl: tpl }));
        } else {
          compiled.push(jst.compilers[jst.compiler].tpl({
            ns: jst.namespace, name: name, tpl: tpl }));
        }
      });
